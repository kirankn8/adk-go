...
// RunSkillScriptArgs accepts argv under either args_list or a single args
// string (the latter is split on whitespace).
type RunSkillScriptArgs struct {
	SkillName  string   `json:"skill_name"            jsonschema:"The name of the skill."`
	ScriptPath string   `json:"script_path"           jsonschema:"Relative path to the script under the skill (e.g. scripts/discover-environment.sh)."`
...
func RunSkillScript(source skill.Source, skillsRoot string, executor code_executors.CodeExecutor) (tool.Tool, error) {
	return functiontool.New(
		functiontool.Config{
			...
			Description: "Executes a script from a skill's scripts/ directory. Pass skill_name + script_path; optionally args_list or a single args string.",
		},
