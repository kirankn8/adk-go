...
func runScriptTool(t *testing.T, tl any, args map[string]any) map[string]any {
	t.Helper()
...
func TestRunSkillScript_DocRedirect(t *testing.T) {
	...
	tl, err := skilltool.RunSkillScript(src, "/tmp/unused", nil)
	if err != nil {
		t.Fatal(err)
	}
	got := runScriptTool(t, tl, map[string]any{"skill_name": "s1", "script_path": "references/how-to.md"})
...
func TestRunSkillScript_SanitizesEscapeAttempt(t *testing.T) {
	...
	got := runScriptTool(t, tl, map[string]any{"skill_name": "s1", "script_path": "../../etc/passwd"})
...
func TestRunSkillScript_NotFoundDidYouMean(t *testing.T) {
	...
	got := runScriptTool(t, tl, map[string]any{"skill_name": "s1", "script_path": "scripts/deplo.sh"})
	if got["error_code"] != "SCRIPT_NOT_FOUND" {
		t.Fatalf("error_code: got %v, want SCRIPT_NOT_FOUND", got["error_code"])
	}
	if got["did_you_mean_path"] != "scripts/deploy.sh" {
		t.Errorf("did_you_mean_path: got %v, want scripts/deploy.sh", got["did_you_mean_path"])
	}
}

func TestRunSkillScript_UnknownSkillDidYouMean(t *testing.T) {
	src := &mockSource{
		frontmatters: []*skill.Frontmatter{{Name: "deployment"}, {Name: "monitoring"}},
	}
	tl, err := skilltool.RunSkillScript(src, "/tmp/unused", nil)
	if err != nil {
		t.Fatal(err)
	}
	got := runScriptTool(t, tl, map[string]any{"skill_name": "deploymnt", "script_path": "scripts/x.sh"})
	if got["error_code"] != "SKILL_NOT_FOUND" {
		t.Fatalf("error_code: got %v, want SKILL_NOT_FOUND", got["error_code"])
	}
	if got["did_you_mean"] != "deployment" {
		t.Errorf("did_you_mean: got %v, want deployment", got["did_you_mean"])
	}
}
